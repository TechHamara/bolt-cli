...
final class LocalRepository implements Repository {
  ...
  /// Creates a local repository at the default location under the Rush home directory.
  factory LocalRepository.defaultLocation({String id = 'local'}) {
    final env = Platform.environment;
    final String rushHome;
    if (env.containsKey('BOLT_HOME')) {
      rushHome = env['BOLT_HOME']!;
    } else if (env.containsKey('BOLT_DATA_DIR')) {
      rushHome = env['BOLT_DATA_DIR']!;
    } else {
      final home = env['HOME'] ?? env['USERPROFILE'] ?? '.';
      rushHome = p.join(home, '.bolt');
    }

    final repoPath = p.join(rushHome, 'repository');
