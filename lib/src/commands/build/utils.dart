import 'dart:convert';
import 'dart:io' show File, Directory, Platform, Process, FileSystemEntity, FileSystemEntityType;
...
class BuildUtils {
  ...
  static Future<void> extractAars(Iterable<String> aars) async {
    for (final aar in aars) {
      ...
      // Extract local AARs in .bolt/build/extracted-aars dir, whereas remote AARs
      ...
      if (assetsDir.existsSync()) {
        _lgr.warn('Dependency ${p.basename(aar)} contains "assets" directory, which is not supported in App Inventor extensions.');
      }
      final jniDir = Directory(p.join(dist, 'jni'));
      if (jniDir.existsSync()) {
        _lgr.warn('Dependency ${p.basename(aar)} contains "jni" directory (native libraries), which is not supported in App Inventor extensions.');
      }
    }
  }
