...
class Compiler {
  ...
  static Future<void> patchAnnotationJarIfNecessary(
    ...
    if (haveLocal) {
      ...
      classpathJars.removeWhere((j) {
        ...
        return name.startsWith('annotations') ||
            norm.contains('/techhamara/bolt/annotations/');
      });
    ...
    if (haveLocal) {
      ...
      for (final jar in classpathJars) {
        ...
        // Accept either the traditional artifact name prefix (rush-annotations)
        // or the canonical Maven repository path containing
        // .../techhamara/bolt/annotations/.../annotations-<version>.jar.
        final normPath = jar.replaceAll(r'\\', '/').toLowerCase();
        if ((name.startsWith('annotations') && name.endsWith('.jar')) ||
            (normPath.contains('/techhamara/bolt/annotations/') &&
  ...
  static Future<void> _compileHelpers(
    ...
    _lgr.dbg('Compiling java helper classes.');
    _lgr.dbg('Found ${helperFiles.length} sources.');
    if (java8) {
      _lgr.dbg('Added custom bootclasspath to target Java 8.');
    }
    ...
    if (hasKtFiles) {
      ...
      _lgr.dbg('Successfully created kotlinc arguments');
      ...
      _lgr.dbg('Successfully created javac arguments');
      _lgr.dbg('Calling javac commands.');
    }
    ...
    try {
      ...
      if (hasKtFiles) {
        ...
        _lgr.dbg('javac is successfully executed.');
      }
    } catch (e) {
      rethrow;
    }
  ...
  static Future<void> compileJavaFiles(
    ...
    try {
      _lgr.dbg('Compiling java classes with java helper sources as classpath.');
      _lgr.dbg('Compiling java classes.');
      _lgr.dbg('Found ${javaFiles.length} sources.');
  ...
  static Future<List<String>> _kotlincArgs(
    ...
    if (config != null && config.genDocs) kaptOptions.add('-Kapt-option:gen_docs=true');
    if (config != null && config.autoVersion) {
      kaptOptions.add('-Kapt-option:auto_version=true');
    }
    ...
    final jvmVersion = await _getJvmVersion();
    final addOpens = jvmVersion > 8
        ? [
  ...
  static Future<int> _getJvmVersion() async {
    try {
      final javaExe = BuildUtils.javaExe();
      final res = await Process.run(javaExe, ['-version']);
      final output = res.stderr.toString() + res.stdout.toString();
      final match = RegExp('version "([^"]+)"').firstMatch(output);
      if (match != null) {
        final jreVersion = match.group(1)!;
        final specMatch = RegExp(r'^(\d+)(\.\d+)?').firstMatch(jreVersion);
        if (specMatch != null) {
          return int.tryParse(specMatch.group(1)!) ?? 8;
        }
      }
    } catch (_) {}
    return 8;
