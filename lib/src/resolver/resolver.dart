...
import 'package:bolt/src/resolver/pom.dart';
...
class ArtifactResolver {
  var _mvnRepos = <String>{
    // The default repositories that Rush will consult when resolving
    // coordinates. Historically we included Apache Maven Central and the
    // Google Android mirror; the CLI templates claim JitPack is also part of
    // the defaults.  Add it here so users don’t have to manually opt-in for
    // GitHub-hosted libraries such as `com.github.xxx:…`.
  ...
  late final String? _localGradleRepo;
  late final String? _customGradleLibPath;

  ArtifactResolver({required Set<String> repos}) {
    final fileService = GetIt.I<FileService>();
    _localMvnRepo =
        p.join(fileService.boltHomeDir.path, 'repository').asDir(true).path;
    ...
    if (home != null) {
      final gradlePath =
          p.join(home, '.gradle', 'caches', 'modules-2', 'files-2.1');
      if (Directory(gradlePath).existsSync()) {
        _localGradleRepo = gradlePath;
      } else {
        _localGradleRepo = null;
      }
    } else {
      _localGradleRepo = null;
    }
    ...
    String? customPath;
    if (home != null) {
      final libsDir = Directory(p.join(home, '.bolt', 'libs'));
      ...
      if (customPath == null) {
        final exactPath = p.join(home, '.bolt', 'libs', 'gradle-8.14.5', 'lib');
        if (Directory(exactPath).existsSync()) {
          customPath = exactPath;
        }
      }
    }
    _customGradleLibPath = customPath;
  ...
  Future<File?> _fetchFile(String relativeFilePath) async {
    ...
    // Attempt to resolve from custom Gradle library directory if available
    if (_customGradleLibPath != null) {
      final filename = p.basename(relativeFilePath);
      final candidate = File(p.join(_customGradleLibPath, filename));
    ...
    // Attempt to resolve from local Gradle cache if available
    if (_localGradleRepo != null) {
      final parts = p.posix.split(relativeFilePath);
      if (parts.length >= 4) {
        final filename = parts.last;
        final version = parts[parts.length - 2];
        final artifactId = parts[parts.length - 3];
        final groupId = parts.sublist(0, parts.length - 3).join('.');

        final artifactDir =
            Directory(p.join(_localGradleRepo, groupId, artifactId, version));
        if (artifactDir.existsSync()) {
          try {
            for (final entity in artifactDir.listSync()) {
              if (entity is Directory) {
                final candidate = File(p.join(entity.path, filename));
                if (candidate.existsSync() && candidate.lengthSync() > 0) {
                  await file.parent.create(recursive: true);
                  await candidate.copy(file.path);
                  return file;
  ...
  Future<List<Pom>> _resolvePomAndParents(String? coordinate) async {
    ...
    if (pomFile == null) {
      throw Exception(
          ...
          'Searched: ${_mvnRepos.join(', ')}\n'
          'If this is a GitHub library you may need to add `https://jitpack.io` to the '
          '"repositories" section of your bolt.yml, or ensure the coordinate is correct.');
    }
