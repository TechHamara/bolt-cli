...
class Artifact {
  ...
  String get classesJar {
    ...
    if (packaging == 'aar') {
      ...
      if (p.isWithin(fs.localDepsDir.path, artifactFile)) {
        baseDirPath = p.join(fs.buildAarsDir.path, p.basenameWithoutExtension(artifactFile));
      } else {
        baseDirPath = p.join(p.dirname(artifactFile), p.basenameWithoutExtension(artifactFile));
      }
      final jar = p.join(baseDirPath, 'classes.jar').asFile();
      return jar.path;
    }
