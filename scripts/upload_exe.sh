...
function fetch() {
  echo "$(curl -s -u techhamara:$pat https://api.github.com/repos/techhamara/pack/contents/exe/$1)"
}
...
EOF
  ...
  curl -X PUT -u techhamara:"$pat" \
    ...
    https://api.github.com/repos/techhamara/pack/contents/exe/$1 \
    -d @build/curl.args
}
...
if [ "$OS" = "Windows_NT" ]; then
  ...
  "Darwin x86_64")
    res=$(fetch "mac")
    exeSha=$(echo "$res" | jq '.[0].sha')
    upload "mac/rush" "$(encode false build/bin/bolt)" "$exeSha"
    ;;
  *)
    res=$(fetch "linux")
    exeSha=$(echo "$res" | jq '.[0].sha')
    upload "linux/rush" "$(encode true build/bin/bolt)" "$exeSha"
    ;;
  esac
fi
