package io.github.techhamara.bolt

import com.charleskorn.kaml.Yaml
import com.charleskorn.kaml.YamlConfiguration
import com.google.appinventor.components.annotations.Extension
import io.github.techhamara.bolt.block.DesignerProperty
import io.github.techhamara.bolt.block.Event
import io.github.techhamara.bolt.block.Function
import io.github.techhamara.bolt.block.Property
import io.github.techhamara.bolt.model.BoltYaml
...
class InfoFilesGenerator(private val extensions: List<ExtensionModel>, private val elementUtils: Elements) {
    private val projectRoot = System.getenv("BOLT_PROJECT_ROOT")
    private val rawBuildDir = Paths.get(projectRoot, ".bolt", "build", "raw").apply {
    ...
    fun generateComponentsJson() {
        ...
        for (ext in extensions) {
            ...
            extJsonObj
                ...
                .put("versionName", extAnnotation.versionName.let { if (it.isBlank()) yaml.version ?: "1.0" else it })
                .put("version", extAnnotation.version.let { if (it.isBlank()) yaml.version ?: (0..999).random().toString() else it })
    ...
    fun generateBuildInfoJson() {
        ...
        val boltVersion = System.getenv("BOLT_VERSION")

        for (ext in extensions) {
            val extJsonObj = JSONObject()
                .put("boltVersion", boltVersion)
                .put("type", ext.fqcn)
        ...
        val featureNodes = doc.getElementsByTagName("uses-feature")
        val features = JSONArray()
        if (featureNodes.length > 0) {
            for (i in 0 until featureNodes.length) {
                val element = featureNodes.item(i) as Element
                val nameAttr = element.attributes.getNamedItem("android:name")
                val reqAttr = element.attributes.getNamedItem("android:required")
                if (nameAttr != null) {
                    val feature = JSONObject()
                        .put("name", nameAttr.nodeValue)
                        .put("required", reqAttr?.nodeValue ?: "true")
     ...
     * Get metadata file
     *
     * @return The bolt.yml file's data
     * @throws IOException If the input can't be read for some reason.
     */
    private fun metadataFile(): BoltYaml {
        val boltYml = if (Paths.get(projectRoot, "bolt.yml").exists()) {
            Paths.get(projectRoot, "bolt.yml").toFile()
        } else {
            Paths.get(projectRoot, "bolt.yaml").toFile()
        }

        val config = YamlConfiguration(strictMode = false)
        return Yaml(configuration = config).decodeFromStream(BoltYaml.serializer(), FileInputStream(boltYml))
    }
